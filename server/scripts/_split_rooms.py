...
for room in rooms:
    ...
    for color, xys in xys_by_color.items():
        ...
        layer_1 = None
        for layer in ['bts', 'layer-1', 'layer-2', 'plm_enemies', 'bts-extra']:
            for source_dir in ['_maptroid-sink', 'sm_cache']:
                source = f'.media/{source_dir}/{world.slug}/{layer}/{room.key}'
                ...
                dest = f'.media/{source_dir}/{world.slug}/{layer}/{new_key}'
